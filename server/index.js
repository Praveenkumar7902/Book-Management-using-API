...
booky.post("/books/new", async (req, res) => {
  ...
  return res.status(201).json({
    ...
    //here we need to give the data with this route in postman and retreived this database in mongodb
  });
}
catch (error){
   return res.status(500).json({
...
booky.post("/author/new", async(req, res) =>  {
  ...
  return res.status(201).json({
    success: true,
    author: addNewAuthor,
    message: "New Author Was Created and Added to the Collection 😎!",
  });
}
catch (error){
  return res.status(500).json({
  success: false,
  message: "Failed to add a New Author to the Collection 😢!",
